...
from common.exception import NotFoundException, InvalidInputException, InvalidValueException, AccessDeniedException, \
    AuthenticationFailedException, DuplicateValueException, UnauthorizedException, MethodNotAllowedException
from model.base import HttpResponse

from util.json import json_response

from pyboot.exception import InvalidInputException as PybootInvalidInputException
from pyboot.exception import InvalidStateException as PybootInvalidStateException
from pyboot.exception import AccessDeniedException as PybootAccessDeniedException
from pyboot.exception import AuthFailedException as PybootAuthFailedException
from pyboot.exception import DuplicateValueException as PybootDuplicateValueException
from pyboot.exception import InvalidValueException as PybootInvalidValueException
from pyboot.exception import MultipleRowsFoundException as PybootMultipleRowsFoundException
from pyboot.exception import NotFoundException as PybootNotFoundException

SKIP_PERMISSION_CHECK = ["CUSTOMER"]
    ...
    def api_controller(self):
        def decorator(f):
            ...
            def api_response_handler(*args, **kwargs):
                try:
                ...
                except (BadRequest, InvalidInputException, InvalidValueException, PybootInvalidInputException,
                        PybootInvalidValueException, PybootInvalidStateException) as e:
                ...
                except (NotFoundException, PybootNotFoundException) as e:
                    logging.warning("Not found [%s %s]: %s" % (request.method, request.url, e))
                    return json_response(HttpResponse(code=404, message=str(e))), 404
                except (DuplicateValueException, PybootMultipleRowsFoundException, PybootDuplicateValueException) as e:
                ...
                except (AccessDeniedException, PybootAccessDeniedException, PybootAuthFailedException) as e:
                    ...
                    logging.warning("Method not allowed [%s %s]: %s" % (request.method, request.url, e))
                    return json_response(HttpResponse(code=405, message=str(e))), 405
...
class Permissions(Decorator):
    ...
    #     if user.role in SKIP_PERMISSION_CHECK:
    #         return True
    ...
    #         else:
    #             raise AccessDeniedException("Permission : %s not found for user" % permission)
    #
    #     if not is_permission_found:
    #         raise AccessDeniedException("Couldn't found required permissions : %s for user" % json.dumps(expected_permissions))
    #
    #     return True
